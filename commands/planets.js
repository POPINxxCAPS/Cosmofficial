...
module.exports = {
    name: 'planets',
    aliases: ['planets'],
    description: "List planets and their locations",
    ...
    async execute(message, args, cmd, client, discord, mainGuild, guild) {
        ...
        .addFields(
          { name: 'Planet Name', value: `${planetsInfo[i].DisplayName}` },
          { name: 'X', value: `${Math.round(planetsInfo[i].Position.X)}` },
          { name: 'Y', value: `${Math.round(planetsInfo[i].Position.Y)}` },
          { name: 'Z', value: `${Math.round(planetsInfo[i].Position.Z)}` },
        )
