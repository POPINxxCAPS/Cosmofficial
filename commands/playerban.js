...
module.exports = {
  name: 'playerban',
  aliases: ['banplayer'],
  description: "Ban a player in-game", // Full Recode Needed
    ...
    await players().then(async (result) => {
      ...
      for (i = 0; i < playerData.length; i++) {
        if (playerData[i].DisplayName === searchedGT) {
          await playerData[i].ban(playerData[i].SteamID)
          return message.channel.send(`${playerData[i].DisplayName} Banned in-server.`)
        }
      }
